...
// Loans count only their repayments (a cost); the borrowed cash is spent on the opportunity.
// A = 500; −500/1.1 − 500/1.21 = −454.5455 − 413.2231 = −867.7686
check('loan L=1000 r=0 d=10% n=2', C.loanPV(1000, 0, 0.10, 2), -867.7686);
// Repayments at the discount rate are worth exactly the amount borrowed: −1000
check('loan L=1000 r=d=5% n=3', C.loanPV(1000, 0.05, 0.05, 3), -1000);
// A = 1000·0.1/(1−1.1^−2) = 576.1905; −576.1905/1.05 − 576.1905/1.1025 = −548.7528 − 522.6218 = −1071.3746
check('loan L=1000 r=10% d=5% n=2', C.loanPV(1000, 0.10, 0.05, 2), -1071.3746);
check('a loan never adds value (r=0, d=50%): PV < 0', Math.sign(C.loanPV(1000, 0, 0.5, 5)), -1);
...
// Loan 1000 at 5% = d over 2 years → repayments worth −1000
check('opportunity total', C.opportunityPV(opp, settings), 97.5057 + 1886.6213 - 1000);
settings.sp500Rate = 0; // linked rate follows project setting: −1000 + 1000/1.1025 = −92.9705
check('opportunity after S&P → 0%', C.opportunityPV(opp, settings), -92.9705 + 1886.6213 - 1000);
...
check('opportunity with invested initial payout',
  ...
  -92.9705 + 1886.6213 + 907.0295 - 1000);
// Older saved opportunities without the field still calculate
check('opportunity missing initialPayout', C.opportunityPV(opp, settings), -92.9705 + 1886.6213 - 1000);
...
// Loan, interest-only forever: −1000·0.05/0.10 = −500; at 0% → 0 (never repaid); r > d → −1714.29
check('loan forever r=5% d=10%', C.loanPV(1000, 0.05, 0.10, INF), -500);
check('…matches n=2000', C.loanPV(1000, 0.05, 0.10, BIG), -500, 0.01);
check('loan forever r=0 → 0', C.loanPV(1000, 0, 0.10, INF), 0);
check('loan forever r=12% d=7%', C.loanPV(1000, 0.12, 0.07, INF), -1000 * 0.12 / 0.07);
...
// 40000/0.07 − 60000·0.05/0.07 = 571428.5714 − 42857.1429
check('opportunity indefinite', C.opportunityPV(indefOpp, indefSettings), 40000 / 0.07 - 60000 * 0.05 / 0.07);
